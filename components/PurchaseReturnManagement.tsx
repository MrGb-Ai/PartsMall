...
const PurchaseReturnManagement: React.FC<PurchaseReturnManagementProps> = ({ 
    ...
    const handleAddItemClick = () => {
        ...
        if (itemToAdd) {
            ...
            const newItem: PurchaseReturnItem = { itemId: itemToAdd.id, quantity: currentItemSelection.quantity, price: currentItemSelection.price };
    ...
    const handleSaveReturn = (printAfterSave: boolean = false) => {
        ...
        try {
            ...
            if (isEditing) { const updatedReturn = { ...returnToSave, lastModifiedBy: currentUser.username, lastModifiedAt: new Date().toISOString() }; setPurchaseReturns(prev => prev.map(inv => inv.id === returnToSave.id ? updatedReturn : inv)); showNotification('edit'); if (printAfterSave) handlePrint(updatedReturn); }
            else { const createdReturn = { ...returnToSave, createdBy: currentUser.username, createdAt: new Date().toISOString() }; setPurchaseReturns(prev => [...prev, createdReturn]); showNotification('add'); if (printAfterSave) handlePrint(createdReturn); }
            resetForm();
                             ...
                             <div className="lg:col-span-4 relative pb-2 z-[100]">
                                ...
                                <div className="flex items-center space-x-2 space-x-reverse">
                                    <div className="relative flex-grow">
                                        <input type="text" value={supplierSearchQuery} onChange={(e) => { setSupplierSearchQuery(e.target.value); openDropdown('supplier'); }} onFocus={() => openDropdown('supplier')} onBlur={() => setTimeout(() => setIsSupplierSuggestionsOpen(false), 250)} className={inputClass} disabled={isViewing || (isEditing && !canEdit)} placeholder="بحث..." autoComplete="off" />
